function [GN] = get_Z_VanDerWaals(GN, object)
%GET_Z_VANDERWAALS
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if isfield(GN,'gasMixProp') && any(ismember({'gasMixProp','all'},object)) && ~strcmp(GN.gasMixProp.PHYMOD,'standard conditions')
    CONST = getConstants();
    [GN.gasMixProp.Z_n_avg, GN.gasMixProp.V_m_n_avg] = calculate_Z_VanDerWaals(CONST.p_n, CONST.T_n, GN.gasMixAndCompoProp);
...
if ~isempty(p)
    [Z, ~, V_m_2, V_m_3] = calculate_Z_VanDerWaals(p, T, GN.gasMixAndCompoProp);
    [p_iso, V_m_liquid, V_m_vapor] = get_pV_vaporLiquid(T, p, GN.gasMixProp.T_pc, GN.gasMixProp.p_pc, GN.gasMixAndCompoProp);
end
